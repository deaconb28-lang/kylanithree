...
test("a rejection of our REQUEST never charges the documents in it", () => {
  // The Haiku switch sent an `effort` parameter Haiku does not accept. Every batch 400'd, and
  // because an unrecognised 400 charges an attempt, 600 good documents were being condemned for a
  // parameter WE sent. A model or parameter complaint is our configuration, not their text.
  const err = apiError(400, '{"type":"error","error":{"type":"invalid_request_error","message":"This model does not support the effort parameter."}}');
  assert.equal(isInfrastructureFailure(err), true);
});

test("model-not-found is our configuration, not the documents", () => {
  assert.equal(isInfrastructureFailure(apiError(400, "model not found: claude-nonexistent")), true);
  assert.equal(isInfrastructureFailure(new Error("unknown parameter: output_config.effort")), true);
});
