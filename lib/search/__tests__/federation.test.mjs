...
test("a Reddit user found live dedupes against the same user in the corpus", () => {
  // The crawler stores the bare username; the query-time source sends networkId "reddit" with the
  // same bare username. Reddit is one flat site with globally unique names, so unlike Lemmy and
  // Discourse there is nothing to qualify — the risk here is only that the two routes disagree.
  const corpus = personFingerprint({ platform: "reddit", authorHandle: "spez" });
  const live = personFingerprint({ platform: "reddit", authorHandle: "spez" });
  assert.ok(corpus);
  assert.equal(corpus, live);
});

test("a Reddit username fingerprints the same however the endpoint spelled it", () => {
  // Different Reddit endpoints return "name", "u/name" and "/u/name" for the same person.
  const bare = personFingerprint({ platform: "reddit", authorHandle: "spez" });
  assert.equal(personFingerprint({ platform: "reddit", authorHandle: "u/spez" }), bare);
  assert.equal(personFingerprint({ platform: "reddit", authorHandle: "/u/spez" }), bare);
});

test("Reddit and Lemmy usernames do not collide", () => {
  // The same handle on the two most similar-looking networks in the registry.
  const reddit = personFingerprint({ platform: "reddit", authorHandle: "tofu" });
  const lemmy = personFingerprint({ platform: "lemmy", authorHandle: "lemmy.world/tofu" });
  assert.notEqual(reddit, lemmy);
});
...
test("tenure is phrased at the scale it is measured, and never names the platform", () => {
  // The platform used to be appended here, and every call site renders this straight after the
  // venue — so a card read "Hacker News · 17 months on Hacker News".
  assert.equal(describeTenure({ platform: "hn", accountAgeDays: 1 }), "1 day");
  assert.equal(describeTenure({ platform: "bluesky", accountAgeDays: 45 }), "45 days");
  assert.equal(describeTenure({ platform: "lemmy", accountAgeDays: 200 }), "7 months");
  assert.equal(describeTenure({ platform: "stackexchange", accountAgeDays: 3650 }), "10 years");
  for (const platform of ["hn", "bluesky", "lemmy", "stackexchange", "reddit", "github"]) {
    assert.ok(!/ on /.test(describeTenure({ platform, accountAgeDays: 200 })), `${platform} leaked its name`);
  }
});
...
test("a zero-day account is a real answer and is said as one", () => {
  ...
  assert.equal(describeTenure({ platform: "hn", accountAgeDays: 0 }), "0 days");
});
