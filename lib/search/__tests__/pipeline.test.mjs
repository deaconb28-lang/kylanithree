...
import { buildPhrasePool, planWave, PHRASES_PER_WAVE } from "../../../.test-build/lib/search/waves.js";
...
import { leadTier, creditsFor, creditsForLead } from "../../../.test-build/lib/credits/rateCard.js";
import { estimateSearch } from "../../../.test-build/lib/credits/estimate.js";
import { lexicalGate, normalizeForIntent, passesSpamFloor, INTENT_WEIGHT } from "../../../.test-build/lib/search/intent.js";
import { fuseToPeople } from "../../../.test-build/lib/search/rrf.js";
import { finalScore, preEnrichmentScore, assignTier, recencyDecay, needsFallback } from "../../../.test-build/lib/search/finalScore.js";
import { sanitizePlan, fallbackPlan } from "../../../.test-build/lib/search/queryPlan.js";
import { normalizeDocument, looksEnglish } from "../../../.test-build/lib/ingest/normalize.js";
import { pollIntervalForYield } from "../../../.test-build/lib/ingest/pipeline.js";
import { normalizeChoice } from "../../../.test-build/lib/ingest/classify.js";
import { mergeLeads, shallowSurvival } from "../../../.test-build/lib/discover/merge.js";
import { rerank } from "../../../.test-build/lib/discover/rerank.js";
...
test("later waves reach different phrases", () => {
  ...
  // Asserted against the constant, not a literal — this is a tuning knob, and hardcoding its
  // value here just breaks the suite every time the search is widened.
  assert.equal(w0.length, Math.min(PHRASES_PER_WAVE, pool.length));
});
...
// --- credits ----------------------------------------------------------------------------------
// "Charged once for a person, ever" and "never charge for what you didn't deliver" are published
// promises, so the identity function behind them has to survive the same human being spelled
// differently by two different sources.

test("the same person hashes the same however the handle is spelled", () => {
  const a = personFingerprint({ platform: "Reddit", authorHandle: "u/MarketMoth" });
  const b = personFingerprint({ platform: "reddit", authorHandle: "@marketmoth" });
  const c = personFingerprint({ platform: "Reddit", authorHandle: "marketmoth" });
  assert.equal(a, b);
  assert.equal(b, c);
});

test("gmail aliases collapse to one person", () => {
  const plain = personFingerprint({ email: "Deacon.Brantley@gmail.com" });
  const aliased = personFingerprint({ email: "deaconbrantley+kylani@googlemail.com" });
  assert.equal(plain, aliased, "dots and +tags must not create a second chargeable person");
});

test("a non-gmail address keeps its dots", () => {
  assert.notEqual(
    personFingerprint({ email: "first.last@company.com" }),
    personFingerprint({ email: "firstlast@company.com" }),
  );
});

test("an email identifies the same person across platforms", () => {
  const viaEmail = personFingerprint({ email: "a@b.com", platform: "Reddit", authorHandle: "someone" });
  const viaEmailElsewhere = personFingerprint({ email: "a@b.com", platform: "Hacker News", authorHandle: "other" });
  assert.equal(viaEmail, viaEmailElsewhere, "email wins over handle — one human, one charge");
});

test("a person with nothing stable to key on is not chargeable", () => {
  assert.equal(personFingerprint({ platform: "Reddit" }), null);
  assert.equal(personFingerprint({ authorHandle: "orphan" }), null, "a handle with no platform is ambiguous");
  assert.equal(personFingerprint({}), null);
});

test("lead tier rises only with what was actually resolved", () => {
  assert.equal(leadTier({ authorHandle: "x" }), "basic");
  assert.equal(leadTier({ authorHandle: "x", role: "Ops manager" }), "standard");
  assert.equal(leadTier({ authorHandle: "x", role: "Ops manager", email: "a@b.com" }), "verified");
  assert.equal(creditsForLead("basic"), 1);
  assert.equal(creditsForLead("standard"), 3);
  assert.equal(creditsForLead("verified"), 6);
});

test("drafting and analytics are metered at zero, not left unpriced", () => {
  assert.equal(creditsFor("draft.write"), 0);
  assert.equal(creditsFor("draft.rewrite"), 0);
  assert.equal(creditsFor("analytics.read"), 0);
  // Sends stay cheap on purpose — the behaviour that creates value must not be rationed.
  assert.ok(creditsFor("message.email") < creditsForLead("standard"));
});

test("the estimate is a range, and already-charged people make it cheaper", () => {
  const fresh = estimateSearch({ searchableVenues: 12, maxWaves: 6 });
  assert.ok(fresh.leadsHigh > fresh.leadsLow, "must be a range, not a point");
  assert.ok(fresh.creditsHigh > 0);
  assert.match(fresh.summary, /leads, est\. \d+–\d+ credits/);

  const repeat = estimateSearch({ searchableVenues: 12, maxWaves: 6, alreadyChargedFingerprints: 40 });
  assert.ok(repeat.creditsHigh < fresh.creditsHigh, "re-searching a known niche must cost less");
  assert.equal(repeat.leadsHigh, fresh.leadsHigh, "dedup reduces the bill, never the leads shown");
});

test("an estimate with nowhere to search costs nothing", () => {
  const none = estimateSearch({ searchableVenues: 0, maxWaves: 6 });
  assert.equal(none.creditsHigh, 0);
  assert.match(none.summary, /cost nothing/);
});

// --- intent cascade, stage 1 ------------------------------------------------------------------
// Tuned for RECALL: a false positive costs one embedding call, a false negative is a lead nobody
// ever sees. So these assert that real demand gets through, and that only the obvious noise doesn't.

test("the lexical gate lets real demand through", () => {
  const pass = [
    "Does anyone know a tool that can handle dock scheduling? We are drowning in spreadsheets here.",
    "Honestly sick of Airtable for this, the pricing changed again and we are looking for alternatives.",
    "We built our own script to handle carrier arrivals because nothing on the market did it properly.",
    "How do we stop three trucks turning up at the same time? Our current process is a nightmare.",
    "Thinking about migrating away from Notion for client work, it just doesn't scale past ten clients.",
    "Looking to hire someone to manage our inbound scheduling because the spreadsheet has become a job.",
  ];
  for (const text of pass) {
    assert.equal(lexicalGate(text).passed, true, `should pass: ${text.slice(0, 50)}`);
  }
});

test("the lexical gate drops the obvious noise", () => {
  const drop = [
    // Gratitude contains a need marker ("what I needed") but is not demand. Long enough to get
    // past the length floor, so this genuinely exercises the hard-reject path.
    ["Thanks, this is exactly what I was looking for and it solved the whole thing for me today.", "hard_reject"],
    ["+1", "too_short"],
    ["Same here", "too_short"],
    ["Show HN: I built a dock scheduling tool that handles carrier arrival windows automatically", "hard_reject"],
    ["Use discount code SAVE20 for an affiliate deal on the best scheduling software available now", "hard_reject"],
    ["The weather has been unusually mild this week and the harvest came in earlier than anyone expected.", "no_need_marker"],
  ];
  for (const [text, reason] of drop) {
    const r = lexicalGate(text);
    assert.equal(r.passed, false, `should drop: ${text.slice(0, 50)}`);
    assert.equal(r.reason, reason, `wrong reason for: ${text.slice(0, 40)}`);
  }
});

test("a need marker inside a quote belongs to the other author", () => {
  const quoted = "> Does anyone know a tool for this?\n\nNo idea sorry, I have never had to deal with that at all.";
  assert.equal(lexicalGate(normalizeForIntent(quoted)).passed, false, "quoted demand is not this author's demand");
});

test("the spam floor keeps accounts with unknown history", () => {
  assert.equal(passesSpamFloor({}), true, "most sources expose no history — absence must not disqualify");
  assert.equal(passesSpamFloor({ accountAgeDays: 2, postCount: 1 }), false);
  assert.equal(passesSpamFloor({ accountAgeDays: 2, postCount: 40 }), true, "new but active is fine");
  assert.equal(passesSpamFloor({ accountAgeDays: 900, postCount: 1 }), true, "old and quiet is fine");
});

test("intent weights rank switching above merely hurting", () => {
  assert.ok(INTENT_WEIGHT.switching_away > INTENT_WEIGHT.seeking_tool);
  assert.ok(INTENT_WEIGHT.seeking_tool > INTENT_WEIGHT.describing_pain);
  assert.equal(INTENT_WEIGHT.none, 0);
});

// --- reciprocal rank fusion ------------------------------------------------------------------

test("fusion collapses one person's many posts into one lead", () => {
  const fused = fuseToPeople([
    { query: "q1", weight: 1, hits: [{ documentId: "d1", personId: "p1" }, { documentId: "d2", personId: "p1" }] },
    { query: "q2", weight: 1, hits: [{ documentId: "d3", personId: "p1" }] },
  ]);
  assert.equal(fused.length, 1, "one person is one lead however many posts surfaced them");
  assert.equal(fused[0].matchingDocumentCount, 3);
  assert.deepEqual(fused[0].matchedQueries.sort(), ["q1", "q2"]);
});

test("fusion needs no score calibration between retrievers — only rank", () => {
  // Same person, same rank, in two lists. Fusion must not care that one list is BM25 and the other
  // cosine; it only ever reads position.
  const fused = fuseToPeople([
    { query: "lex", weight: 1, hits: [{ documentId: "a", personId: "pa" }, { documentId: "b", personId: "pb" }] },
    { query: "vec", weight: 1, hits: [{ documentId: "b", personId: "pb" }, { documentId: "a", personId: "pa" }] },
  ]);
...
test("matching more queries beats matching one query well", () => {
  const broad = fuseToPeople([
    { query: "q1", weight: 1, hits: [{ documentId: "d1", personId: "broad" }] },
    { query: "q2", weight: 1, hits: [{ documentId: "d2", personId: "broad" }] },
    { query: "q3", weight: 1, hits: [{ documentId: "d3", personId: "broad" }] },
  ]);
  const narrow = fuseToPeople([{ query: "q1", weight: 1, hits: [{ documentId: "n1", personId: "narrow" }] }]);
  assert.ok(broad[0].score > narrow[0].score, "breadth of match is itself a signal");
});

test("query weight carries through to the fused score", () => {
  const heavy = fuseToPeople([{ query: "q", weight: 1, hits: [{ documentId: "d", personId: "p" }] }]);
  const light = fuseToPeople([{ query: "q", weight: 0.2, hits: [{ documentId: "d", personId: "p" }] }]);
  assert.ok(heavy[0].score > light[0].score);
});

// --- final scoring and tiers ------------------------------------------------------------------

test("every point of the final score is attributable", () => {
  const s = finalScore({
    rerankScore: 1,
    intentType: "switching_away",
    postedAt: new Date(),
    contactability: "verified",
    activityScore: 1,
  });
  const summed = s.breakdown.rerank + s.breakdown.intent + s.breakdown.recency + s.breakdown.contactability + s.breakdown.activity;
  assert.ok(Math.abs(s.total - summed) < 1e-9, "total must equal the sum of its parts");
  assert.ok(s.total <= 1.0001, "a perfect lead maxes out at 1");
});

test("recency decays but evergreen pain still counts", () => {
  const now = new Date("2026-08-01");
  const fresh = recencyDecay(new Date("2026-07-31"), now);
  const old = recencyDecay(new Date("2025-08-01"), now);
  assert.ok(fresh > old);
  assert.ok(old > 0.4, "a year-old post is still worth something — pain ages well");
});

test("tiers gate on the drop floor, and verified needs a real contact", () => {
  assert.equal(assignTier(0.2, true), "dropped", "below the floor is never shown and never charged");
  assert.equal(assignTier(0.4, false), "basic");
  assert.equal(assignTier(0.5, false), "standard");
  assert.equal(assignTier(0.8, false), "standard", "a high score without a contact is not verified");
  assert.equal(assignTier(0.8, true), "verified");
});

test("the pre-enrichment pass leaves contactability at zero rather than guessing", () => {
  const before = preEnrichmentScore({ rerankScore: 0.8, intentType: "seeking_tool", postedAt: new Date(), activityScore: 0.5 });
  assert.equal(before.breakdown.contactability, 0);
  const after = finalScore({ rerankScore: 0.8, intentType: "seeking_tool", postedAt: new Date(), activityScore: 0.5, contactability: "verified" });
  assert.ok(after.total > before.total, "ordering should improve as enrichment lands, not change its mind");
});

test("thin coverage triggers the live-crawl fallback", () => {
  assert.equal(needsFallback({ scores: [0.9, 0.8, 0.7, 0.6], distinctSources: 5, requestedCount: 8 }).trigger, false);
  assert.equal(needsFallback({ scores: [0.9], distinctSources: 5, requestedCount: 50 }).trigger, true, "too few strong results");
  assert.equal(needsFallback({ scores: [0.9, 0.8, 0.7], distinctSources: 2, requestedCount: 3 }).trigger, true, "too few sources");
  assert.equal(needsFallback({ scores: [0.5, 0.5], distinctSources: 4, requestedCount: 1 }).trigger, true, "nothing scored well");
});

// --- query planning ---------------------------------------------------------------------------

test("the planner sanitizer snaps recency to the sanctioned buckets", () => {
  const [q] = sanitizePlan([{ text: "dock scheduling mess", kind: "symptom", intentFilter: [], recencyDays: 30, weight: 0.7 }]);
  assert.equal(q.recencyDays, 365, "a 30-day window would silently empty a slow niche");
});

test("the planner sanitizer applies the default intent filter per kind", () => {
  const [incumbent] = sanitizePlan([{ text: "airtable alternative", kind: "incumbent", intentFilter: [], recencyDays: 365, weight: 0.9 }]);
  assert.deepEqual(incumbent.intentFilter, ["switching_away", "evaluating_alternatives"]);
  const [workaround] = sanitizePlan([{ text: "built a spreadsheet", kind: "workaround", intentFilter: [], recencyDays: 1825, weight: 0.6 }]);
  assert.deepEqual(workaround.intentFilter, ["building_workaround"]);
});

test("the planner sanitizer drops empties and duplicates", () => {
  const out = sanitizePlan([
    { text: "same query", kind: "problem", intentFilter: [], recencyDays: 1825, weight: 0.8 },
    { text: "Same Query", kind: "problem", intentFilter: [], recencyDays: 1825, weight: 0.8 },
    { text: "   ", kind: "problem", intentFilter: [], recencyDays: 1825, weight: 0.8 },
  ]);
  assert.equal(out.length, 1);
});

test("the fallback plan searches incumbents the lexicon alone never would", () => {
  const plan = fallbackPlan({
    seekingPhrases: ["spreadsheet alternative"],
    problemPhrases: ["trucks stacking up"],
    incumbents: ["Airtable", "Notion"],
  });
  const texts = plan.map((q) => q.text);
  assert.ok(texts.includes("Airtable alternative"));
  assert.ok(texts.includes("Notion alternative"));
  const incumbent = plan.find((q) => q.kind === "incumbent");
  assert.deepEqual(incumbent.intentFilter, ["switching_away", "evaluating_alternatives"]);
});

// --- ingest normalisation ---------------------------------------------------------------------
// Runs before the classifier and drops 30-50% of raw volume at zero cost. Every document dropped
// here never costs an embedding call, storage, or an index slot.

test("normalisation drops what is too short to carry intent", () => {
  const r = normalizeDocument({
    platform: "hn",
    externalId: "1",
  ...
  assert.ok("drop" in r);
  assert.equal(r.drop, "too_short");
});

test("normalisation drops non-target languages before anything expensive runs", () => {
  const r = normalizeDocument({
    platform: "hn",
    externalId: "2",
    url: "https://example.com/2",
    authorRef: "someone",
    body: "これは日本語のテキストです。".repeat(12),
    postedAt: new Date(),
  });
  assert.ok("drop" in r);
  assert.equal(r.drop, "wrong_language");
});

test("normalisation folds the title into the body so a title-only post still classifies", () => {
  const r = normalizeDocument({
    platform: "hn",
    externalId: "3",
    url: "https://example.com/3",
    authorRef: "someone",
    title: "Does anyone know a decent way to schedule inbound deliveries",
    body: "We keep double booking the same slot and it is costing us real money every single week.",
    postedAt: new Date(),
  });
  assert.ok("doc" in r);
  assert.match(r.doc.body, /Does anyone know/);
  assert.match(r.doc.body, /double booking/);
});

test("identical content hashes identically however it is spaced", () => {
  const base = {
    platform: "hn",
    externalId: "a",
  ...
  const long = "We are looking for a tool that can handle this properly because the spreadsheet keeps breaking.";
  const a = normalizeDocument({ ...base, body: long });
  const b = normalizeDocument({ ...base, externalId: "b", body: `  ${long.replace(/ /g, "  ")}  ` });
  assert.ok("doc" in a && "doc" in b);
  assert.equal(a.doc.contentHash, b.doc.contentHash, "crossposts and mirrors must collapse");
});

test("language detection keeps ambiguous short text rather than guessing", () => {
  assert.equal(looksEnglish("dock scheduling spreadsheet"), true, "too short to judge — keep it");
  assert.equal(looksEnglish(""), false);
});

test("poll interval adapts to real yield", () => {
  assert.equal(pollIntervalForYield(0), 1440, "a dead source should not be hammered");
  assert.ok(pollIntervalForYield(1200) < pollIntervalForYield(60), "a productive source earns a shorter interval");
  assert.equal(pollIntervalForYield(1200), 10);
});

// --- classifier output normalisation -----------------------------------------------------------
// These fields were z.enum, and in production one out-of-vocabulary role ("engineer") made zod
// reject the WHOLE response — destroying 19 good classifications and retrying the batch forever at
// the cost of a full model call each time. They are permissive strings now, coerced here.

test("a known choice passes through untouched", () => {
  assert.equal(normalizeChoice("founder", ["founder", "developer", "unknown"]), "founder");
  assert.equal(normalizeChoice("  Developer  ", ["founder", "developer", "unknown"]), "developer");
});

test("a near miss is rescued when exactly one option matches", () => {
  assert.equal(normalizeChoice("ops manager", ["ops", "sales", "unknown"]), "ops");
  assert.equal(normalizeChoice("software developer", ["developer", "designer", "unknown"]), "developer");
});

test("an ambiguous answer becomes unknown rather than a guess", () => {
  // Contains both "sales" and "ops" — guessing either would be inventing a fact.
  assert.equal(normalizeChoice("sales ops", ["sales", "ops", "unknown"]), "unknown");
});

test("an unrecognised answer becomes unknown instead of failing the batch", () => {
  assert.equal(normalizeChoice("astronaut", ["founder", "developer", "unknown"]), "unknown");
  assert.equal(normalizeChoice("", ["founder", "unknown"]), "unknown");
  assert.equal(normalizeChoice(undefined, ["founder", "unknown"]), "unknown");
});

// --- discover merge policy ---------------------------------------------------------------------
// All three rules exist to avoid betraying someone mid-read, so they are worth pinning down.

const lead = (fp, score, over = {}) => ({
  personFingerprint: fp,
  author: fp,
  platform: "hn",
  venueName: "Hacker News",
  permalink: `https://example.com/${fp}`,
  excerpt: "a".repeat(60),
  postedAt: new Date(),
  matchedFor: [],
  score,
...
test("the same person from two sources is one lead", () => {
  const { leads } = mergeLeads({
    existing: [lead("alice", 0.5)],
    incoming: [lead("alice", 0.9, { platform: "stackexchange", permalink: "https://other.com/x" })],
  });
  assert.equal(leads.length, 1, "dedupe is by person, never by post URL");
  assert.equal(leads[0].score, 0.9, "the stronger evidence post wins");
});

test("a pass-1 lead stays labelled pass 1 even when pass 2 re-finds it", () => {
  const { leads } = mergeLeads({
    existing: [lead("alice", 0.5, { foundInPass: 1 })],
    incoming: [lead("alice", 0.9, { foundInPass: 2 })],
  });
  assert.equal(leads[0].foundInPass, 1, "otherwise shallow survival cannot be measured");
});

test("pass-1 leads get no privilege — a stronger pass-2 lead outranks them", () => {
  const { leads } = mergeLeads({
    existing: [lead("shallow", 0.4, { foundInPass: 1 })],
    incoming: [lead("deep", 0.95, { foundInPass: 2 })],
  });
  assert.equal(leads[0].personFingerprint, "deep");
});

test("a card being read does not move when the list re-ranks", () => {
  const existing = [lead("a", 0.9), lead("reading", 0.5), lead("c", 0.4)];
  const { leads } = mergeLeads({
    existing,
    // A much stronger lead arrives that would otherwise push "reading" down.
    incoming: [lead("newcomer", 0.99, { foundInPass: 2 })],
    pinned: new Set(["reading"]),
  });
  assert.equal(leads[1].personFingerprint, "reading", "the pinned card holds its exact index");
  assert.ok(leads.some((l) => l.personFingerprint === "newcomer"), "and the new lead still lands");
});

test("enrichment fills a contact without disturbing the ranking", () => {
  const { leads } = mergeLeads({
    existing: [lead("alice", 0.9)],
    incoming: [lead("alice", 0.2, { contact: { channel: "email", value: "a@b.com", status: "valid" } })],
  });
  assert.equal(leads[0].score, 0.9, "a weaker duplicate must not demote an existing lead");
  assert.equal(leads[0].contact?.value, "a@b.com", "but its contact is still taken");
});

test("new arrivals are reported so the client can animate them", () => {
  const { added } = mergeLeads({ existing: [lead("a", 0.5)], incoming: [lead("b", 0.6), lead("a", 0.1)] });
  assert.deepEqual(added, ["b"]);
});

test("shallow survival measures whether pass 1 seeded or padded", () => {
  const final = [lead("a", 0.9), lead("b", 0.8), lead("c", 0.7)];
  assert.equal(shallowSurvival(final, ["a", "b"]), 100);
  assert.equal(shallowSurvival(final, ["a", "gone"]), 50);
  assert.equal(shallowSurvival(final, []), null, "no pass-1 leads means the metric is undefined");
});

// --- correcting a wrong inference re-ranks in place -------------------------------------------

test("a correction promotes the leads that match the corrected words", () => {
  const leads = [
    lead("wrong", 0.95, { excerpt: "our warehouse dock is a mess every morning" }),
    lead("right", 0.20, { excerpt: "my invoicing spreadsheet keeps breaking every month" }),
  ];
  const out = rerank(leads, ["invoicing spreadsheet", "spreadsheet keeps breaking"]);
  assert.equal(out[0].personFingerprint, "right", "agreement with the correction beats the old score");
  assert.equal(out[0].matchedFor.length, 2, "and the card can say why it matched");
});

test("a lead matching none of the corrected words is demoted, not dropped", () => {
  const leads = [lead("nomatch", 0.9, { excerpt: "completely unrelated post" }), lead("match", 0.1, { excerpt: "the spreadsheet keeps breaking" })];
  const out = rerank(leads, ["spreadsheet keeps breaking"]);
  assert.equal(out.length, 2, "they are a real person who really said that");
  assert.equal(out[1].personFingerprint, "nomatch");
  assert.deepEqual(out[1].matchedFor, []);
});

test("matchedFor is reported in the founder's own casing, not lowercased", () => {
  const out = rerank([lead("a", 0.5, { excerpt: "our SPREADSHEET Keeps Breaking" })], ["Spreadsheet Keeps Breaking"]);
  assert.deepEqual(out[0].matchedFor, ["Spreadsheet Keeps Breaking"], "matching is case-insensitive, display is not");
});

test("short keywords cannot match everything by accident", () => {
  // A two-character keyword would appear in almost any excerpt; index alignment with `keywords`
  // is what makes skipping it safe rather than a source of off-by-one labels.
  const out = rerank([lead("a", 0.5, { excerpt: "an ordinary sentence about invoicing" })], ["an", "invoicing"]);
  assert.deepEqual(out[0].matchedFor, ["invoicing"]);
});

test("rerank leaves scores alone", () => {
  const out = rerank([lead("a", 0.42, { excerpt: "spreadsheet keeps breaking" })], ["spreadsheet"]);
  assert.equal(out[0].score, 0.42, "a made-up score would be indistinguishable from a real one");
});
